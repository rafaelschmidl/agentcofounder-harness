...
Actions declare input fields, additional when guards, assignments and literal success messages. Input fields are full objects with their own required rules: an action may require a value optional in stored records. Canonical stored fields are not automatically general form fields: transition-assigned values belong in hidden and are entered through action inputs. Do not invent a requirement to edit every stored field in generic CRUD forms. If an inferred view creates that conflict, correct the inference; preserve any explicit source requirement with custom mode when needed. Guards support conjunctions of equals/empty/present; invariants mean when implies must and apply to stored records and successful mutations. For a workflow, state_binding maps every canonical state to hidden-field predicates; each action names its canonical transition_id exactly once. The compiler derives source/target guards. Empty/present predicates on one hidden field may avoid a redundant status enum. Enum choices and conditional invariants can coexist. Preserve predicates and invariants when fixing alignment errors.

...
The failure evidence includes compact assertion messages from the Vitest JSON report. Fix the responsible product code or test, not the evidence. Use unique contextual accessible names and scoped Testing Library queries when multiple controls legitimately share a concept. BLOCK-owned setup cleans the DOM and browser storage between tests and restores Vitest spies/global stubs. Use real storage by default; persistence survives unmount/remount within one test. For storage failures use `vi.spyOn(Storage.prototype, "setItem")`; instance method assignments do not reliably replace JSDOM Storage methods.

...
The failure evidence includes compact assertion messages from the Vitest JSON report. Fix the responsible product code or test, not the evidence. Use unique contextual accessible names and scoped Testing Library queries when multiple controls legitimately share a concept. An alert's visible text is not its accessible name: when several legitimate alerts exist, assert one matching node, for example `expect(screen.getAllByRole("alert").some((node) => /could not save/i.test(node.textContent ?? ""))).toBe(true)`. Do not alternate between an unscoped singular alert and `getByRole("alert", { name: /message/ })`. BLOCK-owned setup cleans the DOM and browser storage between tests and restores Vitest spies/global stubs. Use real storage by default; persistence survives unmount/remount within one test. For storage failures use `vi.spyOn(Storage.prototype, "setItem")`; instance method assignments do not reliably replace JSDOM Storage methods.

A test stack identifies where an expectation observed the failure, not necessarily where the defect lives. If a journey cannot observe required state after a valid action, repair the application or domain behavior. Change the test only when its fixture is demonstrably broken or its selector is ambiguous or inconsistent with accessible UI that already implements the requirement.

...
export default function ownedPaths(pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event, context) => {
    ...
    if (mayAgentWrite(appRoot, ownership, candidate)) return undefined;
    if (context.hasUI) context.ui.notify(`Blocked non-agent-owned write: ${candidate}`, "warning");
    return { block: true, reason: "The builder may edit only AGENT-owned files declared by BuildPlan" };
  });
}
